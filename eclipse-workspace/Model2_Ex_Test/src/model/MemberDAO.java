...
public class MemberDAO {
	...
	public List<MemberDTO> getMemberList () {
		...
		try {
			...
			String sql ="SELECT * FROM MEMBER_SESSION";
	...
	public void getInsert(MemberDTO mdto) {
		...
		try {
			...
			sql.append("INSERT INTO MEMBER_SESSION ");
	...
	public MemberDTO getMemberDetail(String userid) {
		...
		try {
			String sql="SELECT * FROM MEMBER_SESSION WHERE userid=?";
			pstmt = con.prepareStatement(sql);
			///??
	...
	public void getUpdate(MemberDTO mdto) {
		...
		try {
			//sql확장시 오타방지
			StringBuilder sql = new StringBuilder();
			sql.append("UPDATE MEMBER_SESSION SET ");
	...
	public void getDelete (String userid) {
		...
		getCon();
		try {
			String sql="DELETE * FROM MEMBER_SESSION WHERE userid=?";
			pstmt = con.prepareStatement(sql);
			///??
			pstmt.setString(1, userid);
			pstmt.executeUpdate();
	...
	public String getLoginCheck (MemberDTO mdto) {
		...
		getCon();
		try {
			String sql="SELECT * FROM MEMBER_SESSION WHERE userid=? AND userpass=?";
			pstmt = con.prepareStatement(sql);
			//?
			pstmt.setString(1, mdto.getUserid());
			pstmt.setString(2, mdto.getUserpass());
